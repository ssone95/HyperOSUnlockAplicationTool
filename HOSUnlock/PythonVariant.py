...
# Gets current Beijing time from NTP
def get_initial_beijing_time():
    client = ntplib.NTPClient()
    beijing_tz = pytz.timezone("Asia/Shanghai")
    for server in ntp_servers:
        try:
            print(col_y + f"\nGetting current Beijing time" + Fore.RESET)
...
# Waits until target time accounting for ping
def wait_until_target_time(start_beijing_time, start_timestamp):
    ...
    print (col_g + f"[Shift set]: " + Fore.RESET + f"{feed_time_shift:.2f} ms.")
    ...
    while True:
        ...
        if time_diff.total_seconds() > 1:
            ...
            print(f"Time reached: {current_time.strftime('%Y-%m-%d %H:%M:%S.%f')}. Starting request submission...")
            break
        else:
            time.sleep(0.0001)

# Checks if account unlock is possible via API
def check_unlock_status(session, cookie_value, device_id):
    try:
        ...
        if is_pass == 4:
            if button_state == 1:
                    print(col_g + f"[Account Status]: " + Fore.RESET + f"request submission is possible.")
                    return True
                ...
                print(col_g + f"[Account Status]: " + Fore.RESET + f"account was created less than 30 days ago.")
            ...
            print(col_g + f"[Account Status]: " + Fore.RESET + f"request was approved, unlock is possible until " f"{deadline_format}.")
            input(f"Press Enter to close...")
            exit()
        else:
...
def main():
    ...
    if check_unlock_status(session, cookie_value, device_id):
        ...
        if start_beijing_time is None:
            print(f"Failed to establish initial time. Press Enter to close...")
            input()
            exit()
            ...
            while True:
                ...
                try:
                    ...
                    if code == 0:
                        ...
                        if apply_result == 1:
                            print(col_g + f"[Status]: " + Fore.RESET + f"Request was approved, checking status...")
                            check_unlock_status(session, cookie_value, device_id)
                        elif apply_result == 3:
                            deadline_format = data.get("deadline_format", "Not specified")
                            print(col_g + f"[Status]: " + Fore.RESET + f"Request was not sent, limit reached. Try again on {deadline_format} (Month/Day).")
                            input(f"Press Enter to close...")
                            exit()
                        elif apply_result == 4:
                            deadline_format = data.get("deadline_format", "Not specified")
                            print(col_g + f"[Status]: " + Fore.RESET + f"Request was not sent, block imposed until {deadline_format} (Month/Day).")
                            input(f"Press Enter to close...")
                            exit()
                    elif code == 100001:
                        print(col_g + f"[Status]: " + Fore.RESET + f"Request was rejected, request error.")
                        ...
                        print(col_g + f"[Error]: " + Fore.RESET + f"Response does not contain required code.")
                        print(col_g + f"[Full response]: " + Fore.RESET + f"{json_response}")
